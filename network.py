...
class Network(nn.Module):
    def __init__(self, dim: int, width: int, height: int, network: Networks):
        ...
        if network == Networks.Small:
            ...
            self.model = nn.Sequential(nn.Conv2d(dim, 128, 5), nn.LeakyReLU(), nn.Conv2d(128, 64, 3), nn.LeakyReLU(), nn.Flatten(), nn.Linear(64 * (width - 6) * (height - 6), 128), nn.LeakyReLU(), nn.Linear(128, 256), nn.LeakyReLU(), nn.Linear(256, height * width), nn.Flatten())
