...
def test_simulator():
    with Load("Agent_Kobo", num=0, isLocal=True) as load:
         with Load("gold_9x9", num=1) as load2:
            ...
            lossboard = [[0 for _ in range(10)] for _ in range(len(env.layers.layers))]
            lossRD = [0 for _ in range(10)]
            for i in range(10):
                for frame in loop(env, collector, teleporter=teleporter):
                    ...
                    lossRD[i] += (torch.sum((guessRD[:,0] - normal_rewards)**2).item())/(guessRD.shape[0] * 1000)
                    for j in range(len(env.layers.layers)):
                        lossboard[j][i] += torch.sum((guessboard.reshape(-1, 4, env.layers.height, env.layers.height)[:,j] - ((board_after - board_before)[tele_dones == 0]**2)[:,j])**2).item()/(guessboard.shape[0] * 1000)
                    if frame % 1000 == 0:
                        print(i)
                        break
            x_values = [((x+1)/15 + 0.1) for x in list(range(10))]
